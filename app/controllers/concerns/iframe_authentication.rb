...
module IframeAuthentication
  ...
  def authenticate_from_referer
    ...
    Rails.logger.error "#{self.class.name}: Authentication failed - no token found. Params: #{params.keys}, Session has token: #{session[:auth_token].present?}, Referer: #{request.referer}"
